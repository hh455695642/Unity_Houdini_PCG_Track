...
Assert-Contains $content 'Test-CodexCliHoudiniMcpConfig' 'Missing full Codex CLI configuration parsing validation.'
Assert-Contains $content 'mcp get houdini' 'Preflight must ask Codex CLI to resolve the Houdini MCP entry.'
Assert-Contains $content 'Test-McpToolDiscovery' 'Missing Houdini MCP protocol tool discovery validation.'
Assert-Contains $content "method = 'tools/list'" 'Preflight must verify that Houdini MCP exposes tools.'
Assert-Contains $content 'McpToolCount' 'Preflight result must report discovered Houdini tool count.'
