...
namespace PCGBike.Editor.Buildings
{
    ...
    /// creates the phase-4 DesignPreset showcase. The primitive meshes are an
    /// authoring reference that artists can replace one Prefab at a time.
    ...
    public static class StreetBuildingProjectOwnedStyleBuilder
    {
        ...
        [MenuItem("PCG/StreetBuilding/Project Owned/Build Styles + Six Building Showcase", priority = 2260)]
        public static void BuildStylesAndShowcaseMenu()
        {
            try
            {
                ...
                Debug.Log("STREETBUILDING_PHASE4_PASS|two project-owned styles and six DesignPreset buildings saved.");
            }
            catch (Exception exception)
            {
                Debug.LogError("STREETBUILDING_PHASE4_FAIL|" + exception);
            }
        }
        ...
        private static void BuildShowcase(StyleAssets brick, StyleAssets stucco)
        {
            ...
            {
                Debug.Log("STREETBUILDING_PHASE4_SKIP|complete six-building showcase already exists.");
                return;
            }
        ...
        private static Material EnsureMaterial(string materialPath, string texturePath, Color color)
        {
            ...
            if (texture == null)
            {
                ...
                Color dark = Color.Lerp(color, Color.black, .12f);
                for (int y = 0; y < 4; y++)
                for (int x = 0; x < 4; x++)
                    texture.SetPixel(x, y, ((x + y) & 1) == 0 ? color : dark);
