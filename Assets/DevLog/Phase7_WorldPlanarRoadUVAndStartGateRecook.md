...
本文不是 Track/Terrain 全量快照。Track 曲线输入、赛道横倾、Adaptive Sampling、拆分输出、起点 Prefab 输出接口，以及 Terrain 自适应土方能力以 Phase 1～Phase 6 日志为准。
...
| Track 参数 | 启用赛道横倾、设计速度调至 35 km/h、路宽调至 21 m | [已验证] |
...
这里是单轴 XZ Planar Projection，不是 Triplanar。道路和路肩通常接近水平面，因此成本远低于三向投影；极端纵坡或大横倾角仍可能出现沿表面方向的投影压缩，需在极端赛道样例中观察。
...
|---|---:|---:|---|
| `enable_track_lateral_tilt` | Off | On | 启用既有赛道横倾链路 |
| `lateral_tilt_design_speed_kph` | 25 | 35 | 以更高设计速度计算目标横倾 |
...
赛道横倾、起点实例输出和道路拆分能力都来自既有 `Track.hda`；提交 7 只在 Unity 场景中启用和调参，没有修改 HDA 定义。
...
三份 Mesh 仍为 16-bit Index Format，顶点数远低于 65,535。增长与赛道横倾/设计速度/路宽组合调参后的 Recook 一致，但当前 Houdini 离线，无法进一步读取 `road_sample_count`、横倾统计或确定具体由哪个参数贡献多少采样点。

Road/Shoulder/Collision 的 Y Extent 同时增大，符合赛道横倾打开后道路横倾与局部高度范围扩大的预期。该结论来自 Unity Mesh AABB，不替代 Houdini 曲率/横倾数值验收。
...
Track Cook 次数明显增加，与赛道横倾、道路宽度、起点 Prefab、材质绑定和多次 Reload/Recook 调试一致。
...
|---|---|---|---|
| 编辑器生成 | 赛道横倾、采样、Sweep、拆分输出、Prefab 点实例 | 仅预览 | Track Cook 次数和 Mesh 顶点增长 |
...
- 无法读取当前 `.hip`、`/obj/Track1`、Cook error/warning、横倾统计、道路 metadata 或起点输出点属性。

由于提交 7 没有修改 HDA/HIP，本日志可通过 Git + Unity 还原提交内容；但以下验证必须在 Houdini 恢复后补做：

1. 强制 Cook `Track1` 并确认 0 error/warning。
2. 读取 `road_sample_count`、横倾最大角度/过渡和碰撞输出统计。
...
| 赛道横倾场景启用 | 已完成配置 | 35 km/h；Houdini 数值输出待复验 |
...
1. Houdini 恢复后补跑 Track 强制 Cook，记录赛道横倾、Adaptive Sampling、输出 Mesh 与起点点实例 metadata。
...
5. 在极端横倾/纵坡样例中检查 XZ Planar 压缩；优先调整材质 Tile，而不是增加 Triplanar 三倍采样。
